...
async def create_server(server: ServerResponse):
    ...
    result = await database.server_collection.insert_one(server_dict)
...
async def get_servers(owner_id: str):
    servers = await database.server_collection.find({"owner_id": ObjectId(owner_id)}).to_list(100)
    return servers
...
async def delete_user(id: str):
    try:
        result = await database.server_collection.delete_one({"_id": ObjectId(id)})
